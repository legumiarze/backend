...
export class MigrationService {
    ...
    async migrate() {
        ...
        await this.neo4jService.run(`
            LOAD CSV WITH HEADERS FROM 'file:///routes.txt' AS csv
            CREATE (r:Route {routeId: toInteger(csv.route_id), routeShortName: csv.route_short_name, routeLongName: csv.route_long_name, routeDesc: csv.route_desc, routeType: toInteger(csv.route_type), routeColor: csv.route_color, routeTextColor: csv.route_text_color});
        `);

        await this.neo4jService.run(`
            LOAD CSV WITH HEADERS FROM 'file:///calendar_dates.txt' AS csv
            MATCH (t:Trip {serviceId: toInteger(csv.service_id)})
            CREATE (t)-[:RUNS_DURING]->(cd:CalendarDate{serviceId: toInteger(csv.service_id), date: csv.date, exceptionType: csv.exception_type });
        `);

        await this.neo4jService.run(`
            LOAD CSV WITH HEADERS FROM 'file:///trips.txt' AS csv
            MATCH (r:Route {routeId: toInteger(csv.route_id)})
            MERGE (r)<-[:USES]-(t:Trip {tripId: toInteger(csv.trip_id), serviceId: toInteger(csv.service_id)});
        `);

        await this.neo4jService.run(`
            LOAD CSV WITH HEADERS FROM 'file:///stops.txt' AS csv
            CREATE (s:Stop {stopId: toInteger(csv.stop_id), stopCode: csv.stop_code, stopName: csv.stop_name, stopDesc: csv.stop_desc, stopLat: toFloat(csv.stop_lat), stopLon: toFloat(csv.stop_lon), parentStation: csv.parent_station, locationType: csv.location_type});
        `);

        await this.neo4jService.run(`
            LOAD CSV WITH HEADERS FROM 'file:///stops.txt' AS csv
            WITH csv WHERE NOT (csv.parent_station IS NULL)
            MATCH (ps:Stop {stopId: toInteger(csv.parent_station)}), (s:Stop {stopId: toInteger(csv.stop_id)})
            CREATE (ps)<-[:PART_OF]-(s);
        `);

        await this.neo4jService.run(`
            LOAD CSV WITH HEADERS FROM 'file:///stop_times.txt' AS csv
            MATCH (t:Trip {tripId: toInteger(csv.trip_id)}), (s:Stop {stopId: toInteger(csv.stop_id)})
